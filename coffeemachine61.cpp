...
protected:
    int price; // price (roubles)
    int weight; // weight (kg)
    int power; // power (voltage)
    string category; // category (cameras/laptops etc)
    string colour; // colour of an item

public:
    ConsumerElectronics(int price = 4500, int weight = 3, int power = 230, string category = "Laptops",
                        string colour = "white") :
            price(price), weight(weight), power(power), category(category), colour(colour) {}

    ConsumerElectronics(const ConsumerElectronics &c) :
            price(c.price), weight(c.weight), power(c.power), category(c.category), colour(c.colour) {}
...
class AbstractCoffeeMachine : public ConsumerElectronics {
protected:
...
public:
    AbstractCoffeeMachine(int price = 4500, int weight = 3, int power = 230, string category = "coffeemachines",
            ...
            ConsumerElectronics(price, weight, power, category, colour), canMakeCappuccino(canMakeCappuccino),
            model(model), typeOfCoffee(typeOfCoffee), numberOfCups(numberOfCups) {}

    AbstractCoffeeMachine(const AbstractCoffeeMachine &c) :
            ConsumerElectronics(c), canMakeCappuccino(c.canMakeCappuccino), model(c.model),
...
public:
    ...
    CoffeeMachine(map<string, string> &m) {
        ...
        weight = Helpers::getValue(m, "weight", 5);
        power = Helpers::getValue(m, "power", 100);
